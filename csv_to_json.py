...
def process_stations(df):
    ...
    # Group by station
    for station_id, station_group in tqdm(df.groupby('idempresa'), 
        ...
        if station_id not in stations:
            station = {
                ...
                "coordinates": {
                    "lat": float(first_row['latitud']) if pd.notna(first_row['latitud']) else None,
                    "lng": float(first_row['longitud']) if pd.notna(first_row['longitud']) else None
                },
                "products": {}
            }
        ...
        # Process products
        for (product_id, product_name), product_group in station_group.groupby(['idproducto', 'producto']):
            ...
            for _, price_row in product_group.iterrows():
                price_entry = {
                    "price": float(price_row['precio']) if pd.notna(price_row['precio']) else None,
    ...
    print("Updated CSV with geocoded coordinates saved as 'precios-historicos-updated.csv'")
...
async def validate_and_geocode_stations(stations, concurrent_requests=5):
    ...
    for station_id, station in stations.items():
        ...
        if (coords["lat"] is None or coords["lng"] is None or 
            coords["lat"] == 0 or coords["lng"] == 0):
    ...
    async def geocode_and_update(station_id, station, session):
        address = f"{station['address']}, {station['town']}, {station['province']}, Argentina"
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [geocode_and_update(station_id, station, session)
                 for station_id, station in stations_to_geocode]
        for f in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc="Geocoding stations"):
            await f
    return stations
...
def main():
    ...
    if GEOCODING_ENABLED:
        stations = asyncio.run(validate_and_geocode_stations(stations, concurrent_requests=5))
    else:
        stations = stations
